...
function chest_handler(chest) {
    ...
    if(seconds_since_last_error < 15) {
        console.debug("Drop data sending paused due to error that occurred " + Math.round(seconds_since_last_error) + " seconds ago");
    } else {
        sendIfReady();
    }
...
.then((res) =>  {
    if(res.ok) {
        if(window.aldc_use_upgrade) {
            $.ajax({
                url: 'http://localhost:8081/upgradescript?t='+(new Date).getTime(),
                dataType: 'script',
        ...
        if(window.aldc_use_compound) {
            $.ajax({
                url: 'http://localhost:8081/compoundscript?t='+(new Date).getTime(),
                dataType: 'script',
        ...
        if(window.aldc_use_exchange) {
            $.ajax({
                url: 'http://localhost:8081/exchangescript?t='+(new Date).getTime(),
                dataType: 'script',
        ...
        if(window.aldc_use_market) {
            $.ajax({
                url: 'http://localhost:8081/marketscript?t='+(new Date).getTime(),
                dataType: 'script',
        ...
        if(window.aldc_use_market_master) {
            $.ajax({
                url: 'http://localhost:8081/marketmasterscript?t='+(new Date).getTime(),
                dataType: 'script',
                success: () => game_log('Thank you for contributing your market data!', '#FFFF00'),
                async: false
            });
